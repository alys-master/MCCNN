...
#pts：points   fts：features   qrs：queries  N：点云数量  K：选择的邻近点的数目  D：步长（坐标维度）  P：输入点的数目  C：通道数（深度）  with_X_transformation：是否需要X变换
def xconv(pts, fts, qrs, tag, N, K, D, P, C, C_pts_fts, is_training, with_X_transformation, depth_multiplier,
    ...
    nn_pts_local = tf.subtract(nn_pts, nn_pts_center, name=tag + 'nn_pts_local')  # (N, P, K, 3)  #tf.subtract的功能是做减法,求出相对坐标
    ...
    if fts is None:
        ...
        nn_fts_input = tf.concat([nn_fts_from_pts, nn_fts_from_prev], axis=-1, name=tag + 'nn_fts_input')    #tf.concat函数的作用是拼接
...
class PointCNN:
    def __init__(self, points, features, is_training, setting):
        xconv_params = setting.with_X_transformation
        ...
        if hasattr(setting, 'xdconv_params'):    #hasattr() 函数用于判断对象是否包含对应的属性
